...
define(['./util.js'], function (Util) {
    var Me = {
        ...
        Edit: function (settings) {
            ...
            content += '<input type="button" class="btn btn-default btn-sm" value="X" onclick="Apps.Grid.EditCancelCallback(\'' + prefix + '\')" />';
            content += '</div>';
            content += '<div style="display: inline; position: relative; top: 300px; left: 4px; z-index: 200;">';
            content += '    <input type="button" id="' + prefix + '_GridsEditDialogOkButton" value="OK" class="btn btn-default btn-sm" onclick="Apps.Grid.EditSaveCallback(\'' + escape(JSON.stringify(settings)) + '\',\'' + escape(JSON.stringify(objRowData)) + '\', ' + settings.saveclick.toString() + ');" />';
            content += '</div>';
        ...
        Add: function (settings) {
            ...
            content += '<input type="button" id="' + prefix + '_GridsEditDialogOkButton" value="OK" class="btn btn-default btn-sm" onclick="Apps.Grid.AddSaveCallback(\'' + escape(JSON.stringify(settings)) + '\', ' + settings.saveclick.toString() + ');" />';
            content += '</div>';
            content += '';
            content += '</div>';
        ...
        SaveCallback: function (callback, viewSpan, fieldName) {

            let td = viewSpan.parent();
            Me.ShowHideCell(td[0], false);
            var editcontrol = $(td).find(".editcontrol"); //Should only be one
            var rowdata = unescape(td.parent().attr("rowdata"));
            var rowdataobj = JSON.parse(rowdata);
            let newValue = $(editcontrol[0]).val();

            //Get grid name (in encompassing table)
            let gridId = td.parent().parent().parent().attr('id');

            //Set DOM
            $('#' + gridId + '_ViewFormat_Row0_Col' + fieldName).text(newValue);

            //Set object
            rowdataobj[fieldName] = newValue;

            var existingGrids = Enumerable.From(Apps.Grid.Grids)
                .Where(function (g) { return g.ElementID == gridId; }).ToArray();

            if (existingGrids.length == 1) {

                if (existingGrids[0].Post) {
                    let action = {
                        Args: [],
                        DataString: JSON.stringify(rowdataobj)
                    };

                    existingGrids[0].Post.Refresh(action, null, function (result) {
                        //TODO: Find a way to handle buried async results like this
                    });
                }
            }
        ...
        RowActionsCallback: function (callback, td) {
        ...
        GetTable: function (settings) {
            ...
            var existingGrids = Enumerable.From(Apps.Grid.Grids).Where(function (g) { return g.id == settings.id; }).ToArray();
            if (existingGrids.length == 0) {
                var newGrid = new Me.GridModel(settings.id, settings.post);
                Apps.Grid.Grids.push(newGrid);
            }
        ...
        RefreshHTML: function (settings) {
            ...
            if (settings.columns) {
                ...
                if (settings.title) {
                    ...
                    if (settings.add)
                        rows += '&nbsp;<input id="add' + settings.id + '" onclick="Apps.Grid.AddCallback(' + settings.add.addclick.toString() + ')" value="' + settings.add.text + '" type="button" class="btn btn-sm btn-primary" />';
                }
                ...
                if (settings.tableactions) {
                    ...
                    //rows += '           </button> ';
                    //rows += '           <ul class="dropdown-menu"> ';
                    //rows += '               <li><a href="#" onclick="Apps.Grid.TableActionCallback()">All</a></li> ';
                    //rows += '               <li role="separator" class="divider"></li> ';

                    $.each(settings.tableactions, function (index, action) {

                        rows += '  <li>  <a class="dropdown-item" href="#" onclick="Apps.Grid.TableActionCallback(' + action.actionclick.toString() + ');">' + action.text + '</a></li>';
                        //rows += '           <li><a href="#" onclick="Apps.Grid.TableActionCallback(' + action.actionclick.toString() + ')">' + action.text + '</a></li> ';
                ...
                if (settings.filters) {
                    ...
                    $.each(settings.filters, function (index, filter) {
                        ...
                        rows += '           </button> ';
                        rows += '           <ul class="dropdown-menu"> ';
                        rows += '               <li><a href="#" onclick="Apps.Grid.FilterCallback(' + filter.selectclick.toString() + ',\'0\',\'' + filter.textfield + '\')">All</a></li> ';
                        rows += '               <li role="separator" class="divider"></li> ';
                        $.each(filter.data, function (index, fd) {

                            rows += '           <li><a href="#" onclick="Apps.Grid.FilterCallback(' + filter.selectclick.toString() + ',\'' + eval('fd.' + filter.valuefield) + '\',\'' + filter.valuefield + '\')">' + eval('fd.' + filter.textfield) + '</a></li> ';
                ...
                if (settings.rowbuttons) {
                    ...
                    $.each(settings.rowbuttons, function (index, button) {
                        ...
                        rowbuttons += '<input id="' + settings.id + '_Row' + index + '_RowButton' + '" type="button" class="btn btn-sm btn-warning" style="margin-top:3px;" value="' + button.text + '" onclick="Apps.Grid.RowButtonCallback(' + button.buttonclick.toString() + ',this);" /> ';
                    });
                ...
                $.each(columns, function (index, col) {
                    ...
                    if (col.hidden === undefined || col.hidden === false) { //Check if hidden
                        var columnclick = '';
                        if (col.click)
                            columnclick = 'onclick="Apps.Grid.ColumnCallback(' + col.click.toString() + ')"';
                ...
                $.each(data, function (rowindex, d) {
                    ...
                    if (settings.rowmouseover)
                        rowmouseover = 'onmouseover="Apps.Grid.RowMouseOverCallback(' + settings.rowmouseover.toString() + ', this, \'' + escape(JSON.stringify(Me.RowData)) + '\');"';

                    var rowmouseout = '';
                    if (settings.rowmouseout)
                        rowmouseout = 'onmouseout="Apps.Grid.RowMouseOutCallback(' + settings.rowmouseout.toString() + ', this, \'' + escape(JSON.stringify(Me.RowData)) + '\');"';

                    var rowclick = '';
                    if (settings.rowclick)
                        rowclick = 'onclick="Apps.Grid.RowClickCallback(' + settings.rowclick.toString() + ', this, \'' + escape(JSON.stringify(Me.RowData)) + '\');"';
                    ...
                    if (settings.rowactions) {
                        ...
                        let actionsclickhandler = '';

                        if (settings.rowactionsclick)
                            actionsclickhandler = ' onclick="Apps.Grid.RowActionsCallback(' + settings.rowactionsclick.toString() + '" ';

                        rows += '    <a class="btn btn-warning btn-sm dropdown-toggle" data-bs-toggle="dropdown" ' + actionsclickhandler + ',this);return false;">';
                        rows += '        Actions';
                        rows += '    </a>';
                        rows += '    <ul class="dropdown-menu">';
                        ...
                        $.each(settings.rowactions, function (index, action) {
                            ...
                            //rows += '    <li><a href="#" onclick="Apps.Grid.RowActionCallback(' + action.actionclick.toString() + ',this);">' + action.text + '</a></li> ';
                            rows += '    <li><a class="dropdown-item" href="#" onclick="Apps.Grid.RowActionCallback(' + action.actionclick.toString() + ',this);return false;">' + action.text + '</a></li>';
                    ...
                    if (rowbuttons && rowbuttonorientation === "left") {
                        if (settings.rowbuttons) {
                            ...
                            $.each(settings.rowbuttons, function (rowbuttonindex, button) {
                                rows += '<input id="' + settings.id + '_Row' + rowindex + '_RowButton' + rowbuttonindex + '" type="button" class="btn btn-sm btn-warning" style="margin-top:3px;" value="' + button.text + '" onclick="Apps.Grid.RowButtonCallback(' + button.buttonclick.toString() + ',this);" /> ';
                            });
                    ...
                    $.each(fields, function (index, field) {
                        ...
                        if (cols.length === 1) {
                            ...
                            if (col.hidden === undefined || col.hidden === false) {
                                ...
                                //VIEW (CLICK)
                                if (field.editclick)
                                    editclick = 'onclick="Apps.Grid.EditCallback(' + field.editclick.toString() + ', $(this).parent())"';
                                else
                                ...
                                if (field.saveclick)
                                    editSpan += '      onclick="Apps.Grid.SaveCallback(' + field.saveclick.toString() + ', $(this).parent().parent(),\'' + field.name + '\')" ';
                                editSpan += '   />';

                                //CANCEL
                                editSpan += '   <input type="button" class="gridcancelcontrol btn btn-sm btn-default" value="Cancel" ';
                                if (field.cancelclick)
                                    editSpan += '      onclick="Apps.Grid.CancelCallback(' + field.cancelclick.toString() + ', $(this).parent().parent().parent())" ';
                                else
                                    editSpan += '      onclick="Apps.Grid.CancelCallback(function(){}, $(this).parent().parent().parent())" ';
                                editSpan += '   />';
                                editSpan += '</span></span>';
        ...
        ExistsInCollection: function (elementId) {
            ...
            var existingDialog = Enumerable.From(Apps.Grid).Where(function (d) { return d.ElementID === elementId }).ToArray();
        ...
        RegisterField: function (fieldName, editType, edit, saveFunction) {
            {
                //var editType = 'text';
                //if (fieldName === 'Script')
                //    editType = 'codeeditor';
        ...
        RegisterColumn: function (fieldName, text, formatFunction) {

            if (formatFunction == null) {
                formatFunction = function (obj)
                {
